...
class CalendarWidget(RelativeLayout):
    ...
    def create_month_scr(self, month, toogle_today=False):
        ...
        for week in month:
            for day in week:
                hours = self.study_times[(day[0], day[3], day[4])] if (day[0], day[3], day[4]) in self.study_times else 0

                tbtn = DayNumButton(text=str(day[0]), background_color=self.study_hours_color(hours))
